    ...
    <div class="container-fluid">
        <div class="row">
            <div class="col-sm-12">
										...
										<td >{{ $provedor->correo }}</td>
                                            ...
                                            <td>
                                                <form action="{{ route('provedors.destroy', $provedor->id) }}" method="POST">
                                                    <a class="btn btn-sm btn-primary " href="{{ route('provedors.show', $provedor->id) }}"><i class="fa fa-fw fa-eye"></i> {{ __('Show') }}</a>
                                                    <a class="btn btn-sm btn-success" href="{{ route('provedors.edit', $provedor->id) }}"><i class="fa fa-fw fa-edit"></i> {{ __('Edit') }}</a>
                                                    @csrf
                                                    @method('DELETE')
                ...
                {!! $provedors->withQueryString()->links() !!}
            </div>
        </div>
    </div>
@endsection
